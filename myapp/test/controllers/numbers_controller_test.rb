...
class NumbersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create number" do
    assert_difference('Number.count') do
      post numbers_url, params: { number: { number: @number.number, position: @number.position } }
    end
  ...
  test "should update number" do
    patch number_url(@number), params: { number: { number: @number.number, position: @number.position } }
